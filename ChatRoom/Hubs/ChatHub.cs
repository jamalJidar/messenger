...
using ChatRoom.Services.UriService;
	...
	public class ChatHub : Hub
	{
		...
		public async Task sendMessage(Guid reciverId, string message, Guid? _messageId)
		{
			...
			if (_u != null)
			{
				...
				if (RECIVER != null)
				{await Clients.User(RECIVER.UserId.ToString()).SendAsync("notification");
		...
		//todo => convert files to stream
		//todo => paginate list message 
		...
		public async Task LsitMessage(Guid recid, Guid caller)
		{
			...
			temp = await messageService.LsitMessage(recid: currnetUser.Id, SenderId: recid);
			var messagDecriptList = new List<MessagDecript>();

			if (_messages.Count() > 0)
			{
				_messages.Where(x => x.ReceiverId == currnetUser.Id && x.MessageStatus != MessageStatus.read)
					.ToList().ForEach(m =>
					{
						m.MessageStatus = MessageStatus.read;
						_messages.AddRange(temp);
